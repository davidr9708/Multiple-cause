...
def get_id_value_cols(df, int_cause, data_type_id, inj_diag, explore, drop_p2):
    ...
    # create new group_cols variable for different intermediate causes
...
def run_pipeline(year, source, int_cause, code_system_id, code_map_version_id,
    ...
    if int_cause == 'infectious_syndrome':
        ...
        drop_cols = [x for x in df.columns if 'multiple_cause' in x and 'infectious_syndrome' in x]
        df = df.drop(drop_cols, axis=1)
        Mapper = MCoDMapper('sepsis', code_system_id, code_map_version_id, drop_p2)
        df = Mapper.get_computed_dataframe(df, map_underlying_cause=False)
        print_log_message("Dropping non-sepsis related observations")
        df = df.loc[df['sepsis'] != 'no_sepsis']
